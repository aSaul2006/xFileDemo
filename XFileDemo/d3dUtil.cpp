...
{
	ID3DXMesh* tempMesh;
	LoadXFile(m_FileName, &tempMesh, mMtrl, mTex);
	mMesh = tempMesh;
	
}

void cRenderResource::setMeshMtTx(ID3DXMesh* &tempMesh, std::vector<Mtrl> tempMtrl, std::vector<IDirect3DTexture9*> tempTex)
{
	tempMesh = mMesh;
	tempMtrl = mMtrl;
	tempTex = mTex;
...
ID3DXMesh* cRenderResource::getMesh()
{
	return this->mMesh;
}

std::vector<IDirect3DTexture9*> cRenderResource::getTex()
{
	return this->mTex;
}

std::vector<Mtrl> cRenderResource::getMtrl()
{
	return this->mMtrl;
}